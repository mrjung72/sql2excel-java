...
| 버전 | 일자 | 주요 변경 사항 |
|------|------|---------------|
| **1.0.0** | 2026-07-19 | 최초 버전. CLI 메뉴, 샘플 쿼리, JDBC 드라이버 JAR 경로 지원 |
| **1.0.2** | 2026-07-25 | Tibero 드라이버 클래스명 수정, Eclipse 환경설정, `dbinfo.json`에서 JDBC 드라이버 클래스명 직접 정의 |
| **1.0.5** | 2026-07-25 | XML 쿼리 파일의 `sheet`/`query` 속성 처리, 사용자 배포판 생성 로직, `dependency-reduced-pom.xml` 생성 방지 |
| **1.0.6** | 2026-07-25 | DB 드라이버 적용, 드라이버 JAR 파일명 적용 |
| **1.0.7** | 2026-07-25 | `mydb.sqlite`, `datas/` 추가 |
| **1.0.8** | 2026-07-25 | 오류 수정 |
| **1.1.0** | 2026-07-25 | 엑셀 시트 스타일 적용 기능 추가 |
| **1.1.1** | 2026-07-25 | `excel-style.json` 위치를 `config/` 아래로 변경 |
| **1.1.2** | 2026-07-25 | 목차(TOC) 시트 생성 기능 추가 |
| **1.1.3** | 2026-07-25 | MariaDB와 MySQL DB 접속 메소드 분리 |
| **1.1.4** | 2026-07-25 | SQL 쿼리문에 줄바꿈 문자 적용 |
| **1.1.5** | 2026-07-25 | 목차 시트에서 각 시트명에 하이퍼링크 적용 |
| **1.1.6** | 2026-07-25 | 엑셀 스타일별 파일 생성 기능 추가 |
| **1.1.7** | 2026-07-26 | 모든 스타일을 포함하는 샘플 엑셀파일 생성 기능 추가 |
| **1.1.8** | 2026-07-31 | 암호화 disable 기능 추가 |
| **1.1.9** | 2026-07-31 | 목차 행 높이 50으로 제한 |
| **1.1.10** | 2026-07-31 | 컬럼 숨기기 기능 추가, DB 리소스 스크립트 경로 변경 |
| **1.1.11** | 2026-08-01 | 동적 변수 및 쿼리문 참조 기능 구현 |
| **1.1.12** | 2026-08-01 | 날짜 컬럼 출력 포맷 설정 기능 추가 |
| **1.1.13** | 2026-08-01 | 정의된 모든 작업 DB에 접속 가능 여부 체크 기능 추가 |
| **1.2.0** | 2026-08-01 | CSV, JSON, XML 파일 생성 기능 추가 |
| **1.2.1** | 2026-08-01 | 컬럼명을 element name으로 적용한 XML 버전 생성, XML 형식 2가지 지원 |
| **1.2.2** | 2026-08-01 | 엑셀 Body 스타일을 `default`/`number`/`date` 3가지로 세분화, 숫자/날짜 서식 추가 |
| **1.2.3** | 2026-08-01 | 엑셀 시트 스타일 속성 적용 기능 개선 |
| **1.2.4** | 2026-08-07 | 동적 시트(`dynamic-sheets`) 생성 기능 추가 |
